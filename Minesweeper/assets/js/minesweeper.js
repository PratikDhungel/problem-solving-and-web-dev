...
function getPossiblePositionAtGivenIndex(i, j){
    ...
    let allPossiblePositions = [[i-1, j-1], [i, j-1], [i+1, j-1],
    [i-1, j], [i+1, j],
    ...
    while(k<allPossiblePositions.length){
        ...
        if(allPossiblePositions[k][0] >= 0 && allPossiblePositions[k][0] < gridLength && allPossiblePositions[k][1] >= 0 && allPossiblePositions[k][1] < gridLength){
